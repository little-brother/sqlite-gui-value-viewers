// Based on https://github.com/HumanGamer/MP3Detect
#ifndef MP3DETECT_H
#define MP3DETECT_H
...
size_t internal_mp3detect_calculate_mp3_frame_size(struct Internal_MP3Detect_Header *head) {
	...
	return (size_t) (((coefficients[head->lsf][head->layer] * head->bitrate / head->samplesPerSec) + head->paddingSize)) * slotSizes[head->layer];
}

BYTE internal_mp3detect_is_mp3_header_mono(struct Internal_MP3Detect_Header *header) {
...
char internal_mp3detect_init_mp3_header(struct Internal_MP3Detect_Header *head, const unsigned char *header) {
	...
	head->lsf = head->version == MPEG1 ? 0 : 1;
	...
	BYTE bitrateIndex = (unsigned char) ((header[2] >> 4) & 0x0F);
	...
	BYTE bIndex = (unsigned char) ((header[2] >> 2) & 0x03);
	if (bIndex == 0x03)
		return 0;
	...
	if (head->layer == Layer2) {
		if (head->version == MPEG1) {
			...
			switch (head->bitrate / 1000 / (internal_mp3detect_is_mp3_header_mono(head) ? 1 : 2)) {
				case 32:
				case 48:
					head->allocationTableIndex = head->samplesPerSec == 32000 ? 3 : 2;
					break;
				case 56:
				case 64:
				case 80:
...
char internal_mp3detect_read_mp3_header(struct Internal_MP3Detect_Header *head, const unsigned char *rawdata,
	size_t len, size_t *offset, char exactOffset, struct Internal_MP3Detect_Header *lastHeader) {
	head->allocationTableIndex = 0;
	head->bound = 32;

	int step = 1;
	while (*offset < len) {
		const unsigned char* data = rawdata + *offset;
...
void internal_mp3detect_skip_id3_tags(const unsigned char *rawdata, size_t *offset) {
	const unsigned char* data = rawdata + *offset;
...
char internal_mp3detect_read_mp3_frame(const unsigned char* rawdata, size_t len,
	size_t *offset, char findSubFrames, char exactOffset, struct Internal_MP3Detect_Header *compareHeader) {

	struct Internal_MP3Detect_Header header;
	if (!internal_mp3detect_read_mp3_header(&header, rawdata, len, offset, exactOffset, compareHeader))
		return 0;
	...
	if (findSubFrames)
	{
		...
		return internal_mp3detect_read_mp3_frame(rawdata, len, &newOffset, 0, 1, &header);
	}

	return 1;
}

char is_mp3(const unsigned char * rawdata, size_t len) {
	size_t offset = 0;
	internal_mp3detect_skip_id3_tags(rawdata, &offset);
	return internal_mp3detect_read_mp3_frame(rawdata, len, &offset, 1, 0, NULL);
}
